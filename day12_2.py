...
def counter_lower(current_path):
    full_counter = dict(Counter(current_path))
    keys_to_remove = []
    for key in full_counter.keys():
        if not key.islower() or key == "start" or key == "end":
            keys_to_remove.append(key)
    
    for key in keys_to_remove:
        full_counter.pop(key)
...
def search_path(current_position, map_dict, current_path, all_paths):
    ...
    for next_coordinate in map_dict[current_position]:
        if next_coordinate != "start" and (not next_coordinate.islower() or (next_coordinate not in current_path or 2 not in counter_lower(current_path).values()) ):
